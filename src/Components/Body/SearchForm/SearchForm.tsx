...
function SearchBox({getSpecificBreed}:SearchProps) {
  const [searchPrompt, setSearchPrompt] = useState('')

  const triggerSearch = (prompt: string) => {
    console.log('searching with prompt:', prompt)
    getSpecificBreed(prompt);
  }

  return (
    <>
    <select className="dog-selector" 
    onChange={event => setSearchPrompt(event.target.value)}>
      ...
      <button onClick={()=> {triggerSearch(searchPrompt)}}>Search!</button>
    </>
  )
}
